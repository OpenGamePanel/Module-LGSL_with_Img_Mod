  ...
  if (!function_exists('lgsl_url_path')) { // START OF DOUBLE LOAD PROTECTION
      ...
      function lgsl_group_totals($server_list = false)
      {
          if (!is_array($server_list)) {
              $server_list = lgsl_query_group(array( "request"=>"sc" ));
          }
    ...
    "images/icons/{$type}.png");
      ...
      function lgsl_image_map($status, $type, $game, $map, $check_exists = true, $id = 0)
      {
          ...
          if ($status) {
              return get_map_path($type, $game, $map);
          } else {
              return "modules/lgsl_with_img_mod/lgsl_files/other/map_no_response.jpg";
          }
      }
      ...
      function lgsl_sort_servers($server_list)
      {
          ...
          if ($lgsl_config['sort']['servers'] == "id") {
              usort($server_list, "lgsl_sort_servers_by_id");
          } elseif ($lgsl_config['sort']['servers'] == "zone") {
              usort($server_list, "lgsl_sort_servers_by_zone");
          } elseif ($lgsl_config['sort']['servers'] == "type") {
              usort($server_list, "lgsl_sort_servers_by_type");
          } elseif ($lgsl_config['sort']['servers'] == "status") {
              usort($server_list, "lgsl_sort_servers_by_status");
          } elseif ($lgsl_config['sort']['servers'] == "players") {
      ...
      {
          if ($server_a['o']['id'] == $server_b['o']['id']) {
              return 0;
          }
      ...
      {
          if ($server_a['o']['zone'] == $server_b['o']['zone']) {
              return 0;
          }
      ...
      function lgsl_sort_players($server)
      {
          ...
          if ($lgsl_config['sort']['players'] == "name") {
              usort($server['p'], "lgsl_sort_players_by_name");
          } elseif ($lgsl_config['sort']['players'] == "score") {
      ...
      {
          if ($word_wrap) {
              $string = lgsl_word_wrap($string, $word_wrap);
          }
